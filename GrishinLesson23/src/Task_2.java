...
public class Task_2 {
    ...
    public static void main(String[] args){
        ...
        long startOfYear = scanner.nextLong(); //1735614000L Начало 2025 года unix

        // нам нужно знать, когда начался этот год
        Instant startOfYearMoment = LocalDateTime.parse(startOfYear + "-01-01T00:00:00.000").toInstant(ZoneOffset.UTC);
        System.out.println(startOfYearMoment.toString());

        // и текущую дату
        Instant thisMoment = Instant.now().plusMillis((long) userTimezone * 60 * 60 * 1000); //Текущая дата по МСК
        System.out.println(thisMoment.toString());

        // и конечную дату нашего графика рассветов и закатов (плюс семь дней)
        Instant lastMoment = thisMoment.plusMillis(7 * 24 * 60 * 60 * 1000);
        System.out.println(lastMoment.toString());
    ...
    private static int dayOfYearFromInstant(Instant startOfYear, Instant time) {
        ...
        return (int) fromStartOfYear / (24 * 60 * 60 * 1000) + 1; //Так как день начинается не с 0, а с 1
    }
